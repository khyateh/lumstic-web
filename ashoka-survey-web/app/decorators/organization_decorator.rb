class OrganizationDecorator < Draper::Decorator
  ...
  def asset_space_in_bytes
    question_space = Question.where("photo_file_size IS NOT NULL AND survey_id IN (?)", surveys.select(:id)).sum("photo_file_size")
    answer_space = Answer.where("photo_file_size IS NOT NULL AND response_id IN (?)", responses.select(:id)).sum("photo_file_size")
