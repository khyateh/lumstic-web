class SurveyReporter
  ...
  def monthly_incomplete_responses_for(user_id)
    incomplete_hash = @survey.responses.where(:user_id => user_id, :status => Response::Status::INCOMPLETE).group("to_char(created_at, 'Mon YYYY')").count
  ...
  def monthly_complete_responses_for(user_id)
    complete_hash = @survey.responses.where(:user_id => user_id, :status => Response::Status::COMPLETE).group("to_char(completed_at, 'Mon YYYY')").count
  ...
  def total_incomplete_responses
    total_incomplete_responses = @survey.responses.where(:status => Response::Status::INCOMPLETE).count
  end

  def total_complete_responses
    total_complete_responses = @survey.responses.where(:status => Response::Status::COMPLETE).count
  end
end